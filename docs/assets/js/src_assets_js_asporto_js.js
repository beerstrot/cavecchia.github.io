...
function mkModal(p, secDiv) {
  ...
  var btnPrice = M('span', 'price', M('button', 'button expanded-with-padding extra-space-button-modal', footer, {
    ...
    $('.input-number-increment-' + pid).click(function () {
      ...
      $input.val(val + 1);
      p.quantity = val + 1;
      $('.prezzo-item-' + pid).html((0,_utils__WEBPACK_IMPORTED_MODULE_0__.formatNum)((val + 1) * p.price1));
      updateTotal();
    });
    $('.input-number-decrement-' + pid).click(function () {
      ...
      $input.val(v);
      p.quantity = v;
      $('.prezzo-item-' + pid).html((0,_utils__WEBPACK_IMPORTED_MODULE_0__.formatNum)(v * p.price1));
      updateTotal();
    });
    closeBtn.click();
  })
...
function updateTotal() {
  ...
  $('.carrello-table-totale').html(" &nbsp;&nbsp;\u20AC&ensp;".concat((0,_utils__WEBPACK_IMPORTED_MODULE_0__.formatNum)(total)));
  var quantity = prods.reduce(function (a, p) {
    return a + p.quantity;
  }, 0);
  $('.prod-quantity').html(" &nbsp;".concat(quantity));
}
